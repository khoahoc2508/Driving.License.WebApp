...
import type { CreateCarCommand, UpdateCarCommand, GetCarsDto } from '@/types/carTypes'
...
interface AddCarDialogProps {
    ...
    onClose: () => void
    onSuccess: () => void
    editData?: GetCarsDto | null
    mode?: DialogMode
}

interface FormData extends Omit<CreateCarCommand, 'id'> {
    id?: string
}
...
const AddCarDialog = ({
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            if (mode === DialogMode.EDIT && editData) {
                const payload: UpdateCarCommand = {
                    id: editData.id!,
                    name: data.name,
                ...
                const response = await carsAPI.UpdateCar(payload)
                ...
                if (response.data.success) {
                    ...
                    toast.error(response.data.message || 'Có lỗi xảy ra khi chỉnh sửa xe')
                }
            } else {
                const payload: CreateCarCommand = {
                    name: data.name,
                    description: data.description,
                    active: data.active
                }

                const response = await carsAPI.CreateCar(payload)
                ...
                if (response.data.success) {
                    ...
                    toast.error(response.data.message || 'Có lỗi xảy ra khi thêm xe')
                }
            }
        } catch (error: any) {
    ...
    const getDialogTitle = () => {
        return mode === DialogMode.EDIT ? 'Chỉnh sửa xe' : 'Thêm xe'
    }
