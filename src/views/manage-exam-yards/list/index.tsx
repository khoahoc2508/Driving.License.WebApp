...
import type { ExamYardListType, GetExamYardsQueryParams, GetExamYardsDto } from '@/types/examYardTypes'
import examYardsAPI from '@/libs/api/examYardsAPI'
import DebouncedInput from '@/components/common/DebouncedInput'
import AddExamYardDialog, { DialogMode } from './AddExamYardDialog'
import Table from './Table'

const ManageExamYardsList = () => {

    const [dataTable, setDataTable] = useState<ExamYardListType>([])
    ...
    const [params, setParams] = useState<GetExamYardsQueryParams>()

    // Modal states
    const [openAddDialog, setOpenAddDialog] = useState(false)
    const [editData, setEditData] = useState<GetExamYardsDto | null>(null)
    ...
    const getExamYards = async (params: GetExamYardsQueryParams): Promise<void> => {
        try {
            setLoading(true)
            const res = await examYardsAPI.GetExamYards(params)
    ...
    return (
        <Card className='h-full flex flex-col'>
                ...
                <Grid container spacing={5} alignItems={'flex-end'}>
                    <Grid size={{ xs: 12, sm: 4, md: 3 }}>
                        <Autocomplete
                            ...
                            id='exam-yard-status-autocomplete'
            ...
            <Table
                ...
                isLoading={isLoading}
                onEditExamYard={handleEditExamYard}
            />
...
export default ManageExamYardsList
